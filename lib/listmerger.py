...
def m3u_merge(notification=False):
	...
	xbmc.sleep(1000)
	out = os.path.join(selfAddon.getSetting('output_folder'),selfAddon.getSetting('file_name_lists').replace('.m3u8','').replace('.m3u','')+'.tmp')
	...
	if xbmcvfs.exists(out): os.rename(out, out.replace('.tmp', '.m3u'))
	return
